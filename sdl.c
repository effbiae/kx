...
ZI tcb(I x,V*y){K z=y;SDL_Event e;SDL_UserEvent f;e.type=f.type=SDL_USEREVENT;f.code=0;f.data1=zK[0];f.data2=y;e.user=f;SA(PushEvent,(&e));R x;}
ZK tim(K x,K y,K z){K d;I i;A(i=SDL_AddTimer(xj,(V*)tcb,d=knk(2,y,z)));R knk(2,ki(i),d);}
...
#define F(m) m(home,1),m(start,1),m(txt,1),m(tx,3),m(txz,1),m(t0,1),m(rcp,2),m(rp,1)
...
I main(I n,S*v){
 ...
 {SDL_Event e;
  ...
  {if(e.type==SDL_QUIT)run=0;
   ...
   }else if(e.type==SDL_USEREVENT){K*d=(K*)&e.user.data1;k(-c,d[0]->s,d[1],(K)0);}
  }
